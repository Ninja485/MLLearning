...
def approx_average_is_average(hand):
    ...
    first_last_mean = (hand[0] + hand[-1])/2
    middle_index = round(len(hand)/2)
    return mean in {first_last_mean,hand[middle_index]}
...
def average_even_is_average_odd(hand):
    ...
    for index in range(0,len(hand)):
        if index % 2 == 0:
            even_index_list.append(hand[index])
        else:
            odd_index_list.append(hand[index])
    return card_average(odd_index_list) == card_average(even_index_list)
